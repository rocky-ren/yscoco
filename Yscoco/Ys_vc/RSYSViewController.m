...
@interface RSYSViewController ()<UIAlertViewDelegate>
{
    bool _canOpen;
}
...
@implementation RSYSViewController
...
-(void)lineAnimation{
    ...
    if (upOrdown == NO) {
        ...
        if (2*num >= MiddleWidth-12) {
            upOrdown = YES;
            _line.image = [UIImage imageNamed:@"VCImge.bundle/Icon_SaoLineOn"];
        }
    }else {
        num --;
        _line.frame = CGRectMake(leadSpace, Top_Height+2*num, MiddleWidth, 12);
        if (num == 0) {
            upOrdown = NO;
            _line.image = [UIImage imageNamed:@"VCImge.bundle/Icon_SaoLine"];
        }
    }
}
...
- (void)creatUI{
    ...
    [backBtn setImage:[[UIImage imageNamed:@"VCImge.bundle/anniu"] imageWithRenderingMode:
    ...
    imageView.image = [UIImage imageNamed:@"VCImge.bundle/Icon_SaoYiSao"];
