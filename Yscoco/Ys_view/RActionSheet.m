...
#pragma mark - 初始化控件
- (instancetype)initWithTitle:(NSString *)title confirms:(NSArray *)confirms cancel:(NSString *)cancel style:(RActionSheetStyle)style
{
    self = [super init];
    if (self) {
        self.title = title;
...
- (void)createSubViews
{
    ...
    for (int i=0; i<self.confirms.count; i++) {
        ...
        [confirm setBackgroundImage:[Util createImageWithColor:[UIColor colorWithRed:240/255.0 green:240/255.0 blue:240/255.0 alpha:1]] forState:UIControlStateHighlighted];
        ...
        if (self.style == RActionSheetStyleDestructive) {
            [confirm setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        }else if(self.style == RActionSheetStyleCancel){
    ...
    [cancel setBackgroundImage:[Util createImageWithColor:[UIColor colorWithRed:240/255.0 green:240/255.0 blue:240/255.0 alpha:1]] forState:UIControlStateHighlighted];
...
{
    index = [self.confirms indexOfObject:button.titleLabel.text];
